...
const PICKUP_SYSTEM_PROMPT_BUILDERS: Record<SupportedLanguage, (targetLabel: string, explainLabel: string) => string> = {
  en: (targetLabel, explainLabel) =>
    `You are a dictionary of ${targetLabel} slang used in live Twitch chat. The user will show you one chat message that actually appeared in a live stream, written in ${targetLabel}. Pick out only the special expressions a learner could not easily guess the meaning of (slang, abbreviations, idioms, memes) and give each a short ${explainLabel} meaning of roughly 3 to 8 words. Every term must be an exact substring copied from the original ${targetLabel} message. Do not include ordinary words, pronouns, numbers, emote names, or @mentions. If you are not sure what an expression means, leave it out rather than guessing. If the message has nothing special, return an empty terms array. Respond only in the requested JSON structure.`,
  ja: (targetLabel, explainLabel) =>
