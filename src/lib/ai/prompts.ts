...
const TRANSLATE_SYSTEM_PROMPT_BUILDERS: Record<SupportedLanguage, (targetLabel: string, explainLabel: string) => string> = {
  en: (targetLabel, explainLabel) =>
    `You are a translator for live Twitch chat. The user will show you one chat message that actually appeared in a live stream, written in ${targetLabel}. Translate the whole message into natural, casual ${explainLabel} that preserves the tone (slang, jokes, excitement). Emotes in the message have been replaced with placeholder tokens such as "[[E0]]"; copy every placeholder into your translation exactly as it is, and keep @mentions and URLs unchanged. Do not add explanations or notes. Respond only in the requested JSON structure.`,
  ja: (targetLabel, explainLabel) =>
    `あなたはTwitchのライブ配信チャットの翻訳者です。ユーザーはライブ配信で実際に流れた${targetLabel}のチャット発言を1件見せます。発言全体を、スラング・冗談・興奮といった口調を保ったまま自然でくだけた${explainLabel}に翻訳してください。発言中のemoteは "[[E0]]" のようなプレースホルダに置き換えてあります。プレースホルダはすべて訳文にそのまま書き写し、@メンション・URLもそのまま残してください。解説や注釈は加えないでください。指定されたJSON構造だけで答えてください。`,
  es: (targetLabel, explainLabel) =>
    `Eres un traductor de chat en vivo de Twitch. El usuario te mostrará un mensaje de chat que realmente apareció en una transmisión en vivo, escrito en ${targetLabel}. Traduce el mensaje completo a un ${explainLabel} natural e informal que conserve el tono (jerga, bromas, entusiasmo). Los emotes del mensaje han sido sustituidos por marcadores de posición como "[[E0]]"; copia cada marcador en tu traducción exactamente igual, y mantén sin cambios las menciones (@) y las URL. No añadas explicaciones ni notas. Responde únicamente con la estructura JSON solicitada.`,
  de: (targetLabel, explainLabel) =>
    `Du bist ein Übersetzer für Twitch-Livechats. Der Nutzer zeigt dir eine Chat-Nachricht, die tatsächlich in einem Livestream auf ${targetLabel} geschrieben wurde. Übersetze die gesamte Nachricht in natürliches, lockeres ${explainLabel} und bewahre dabei den Ton (Slang, Witze, Begeisterung). Die Emotes in der Nachricht wurden durch Platzhalter wie "[[E0]]" ersetzt; übernimm jeden Platzhalter unverändert in deine Übersetzung und lass @-Erwähnungen und URLs unverändert. Füge keine Erklärungen oder Anmerkungen hinzu. Antworte ausschließlich in der angeforderten JSON-Struktur.`,
  fr: (targetLabel, explainLabel) =>
    `Tu es un traducteur pour le chat en direct de Twitch. L'utilisateur te montrera un message de chat qui est réellement apparu dans un stream en direct, écrit en ${targetLabel}. Traduis le message complet en ${explainLabel} naturel et familier, en conservant le ton (argot, blagues, enthousiasme). Les emotes du message ont été remplacés par des marqueurs tels que "[[E0]]" ; recopie chaque marqueur tel quel dans ta traduction, et laisse les mentions (@) et les URL inchangées. N'ajoute ni explications ni notes. Réponds uniquement dans la structure JSON demandée.`,
};
