/**
 * チャンネル視聴ページ(/[channel])。URL のチャンネル名が接続先の起点となる。
 *
 * マウント時(および URL のチャンネルが変わったとき)に、URL のチャンネル名(正規化済み)で
 * chat-connection ストアの `connect` を呼ぶ。既に同じチャンネルへ接続中なら呼び直さない。
 * これによりホームの検索フォーム経由だけでなく、/username への直接アクセスでも視聴を開始できる。
 * 切断(StreamInfoPanel の Disconnect ボタンなど)でストアのチャンネルが null に戻ったら
 * ホーム(/)へ遷移する(ウェルカム画面はホームが表示する)。
 *
 * 画面は、上半分に配信embed(TwitchEmbedPlayer)と配信者情報パネル(StreamInfoPanel)、
 * 下半分に3カラムのチャット閲覧領域を表示する。全体がビューポート1ページに収まり
 * (レイアウト側で高さを固定)、3カラム領域は発言数に関わらず最初から下半分の全域を占める
 * (スクロールは3カラム領域の内部で行う)。
 *
 * 流れてくる発言は3列で表示する:
 *
 ...
 * 言語ペア(学ぶ言語 / 解説言語)のセレクトと設定ダイアログはヘッダー(SiteHeader)に表示する。
 * 除外パターンは bot-filter ストアが LocalStorage から復元し、chat-connection ストアが受信時に適用する。
...
export default function ChannelPage() {
  // URL(/[channel])のチャンネル名が接続先の起点。大文字・先頭の # は IRC 接続時と同じ規則で正規化する
  const params = useParams<{ channel: string }>();
  const channelParam = normalizeChannelName(params.channel);
  ...
  // URLのチャンネルへ接続する(検索フォームからの遷移・/username への直接アクセスの両方をここで処理する)。
  // パイプラインの開始(上の effect)より後に実行されるため、接続直後の発言も取りこぼさない
  useEffect(() => {
    const state = useChatConnectionStore.getState();
    if (state.channel === channelParam && isConnectingOrConnected(state.connectionState)) return;
    state.connect(channelParam);
  }, [channelParam]);

  // 切断(Disconnect ボタンなど)でストアのチャンネルが null に戻ったらホームへ遷移する。
  // 接続(上の effect)は同期的にチャンネルをストアへ反映するため、マウント直後に誤って遷移することはない
  useEffect(() => {
    if (useChatConnectionStore.getState().channel === null) router.replace("/");
  }, [channel, router]);
