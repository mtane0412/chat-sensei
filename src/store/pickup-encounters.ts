 ...
 *
 * ハイブリッド抽出(issue #112)の導入後に実チャット評価(issue #117。2配信・各45分)で再確認した。
 * 全候補が採用されたと仮定したシミュレーションで、候補の再出現の25〜36%が抑制され、再表示は45分間で
 * 0〜3回だった。同じ表現の連続表示は抑えられており、不足を示すデータが無いため30分のままとする
 */
